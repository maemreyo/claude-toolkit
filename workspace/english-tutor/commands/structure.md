...
- `--chunk=<n>`: Number of files to process per agent call (default: 25)
...
4. **Process Files in Batches**
   - Logic: Group the pending files into chunks of size `--chunk` (default: 25)
...
```bash
# Process all pending files (default chunk: 25)
/structure
